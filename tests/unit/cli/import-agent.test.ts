...
describe('import-agent runtime preservation', () => {
  ...
  it('rejects unsupported runtime and defaults to claude-code', async () => {
    const config = await importFixture('invalid-runtime-import', 'custom-runtime-xyz');
    expect(config.runtime).toBe('claude-code');
  });

  it('preserves hermes runtime from the export manifest', async () => {
    const config = await importFixture('hermes-runtime-import', 'hermes');
